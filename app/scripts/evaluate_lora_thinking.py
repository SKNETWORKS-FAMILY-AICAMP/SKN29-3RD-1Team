...
import evaluate_base_thinking as base
from eval_common import (
    DEFAULT_LORA_OUTPUT_ROOT,
    DEFAULT_THINKING_ADAPTER,
    LoraModelRunner,
    add_lora_args,
    build_problem_text,
    read_dataset,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate LoRA Thinking Process only.")
    add_lora_args(parser, DEFAULT_LORA_OUTPUT_ROOT / "thinking", max_new_tokens=384, adapter_path=DEFAULT_THINKING_ADAPTER)
    ...
    rows: List[base.ThinkingRow] = []
    for sample_index, sample in enumerate(samples):
        print(f"[lora thinking {sample_index + 1}/{len(samples)}] generating")

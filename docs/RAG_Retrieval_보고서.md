...
초기 구축 과정에서 algorithm_key 누락 문제가 발견되었다.
...
이를 통해 Metadata Filter의 정확성과 안정성을 확보하였다.
...
- 비전공자도 이해하기 쉬운 설명
- 알고리즘 개념과 예제 코드가 포함된 문서
- 코딩테스트 학습에 직접적으로 도움이 되는 자료
- 중복되거나 품질이 낮은 자료 제외
...
가장 큰 성능 향상 요인: **Metadata Filtering + Document Voting**

Hard Query 기준 Hit@1을 42%에서 90%까지 향상시켰다.
...
구체적으로는 알고리즘 문서 전처리, 메타데이터 설계, 청킹, 임베딩, ChromaDB 저장, Retrieval V1 설계, Metadata Filtering, Query Expansion, MMR Search, Document Voting, Failure Detection, Fallback Query Rewrite, KoNLPy/TF-IDF baseline, Basic/Hard 평가까지 수행했습니다.

그 결과 단순 벡터 검색 대비 검색 성능을 크게 개선했습니다.
...
**Q. 왜 Recall@5를 중요하게 보았나요?**

RAG의 목적은 정답 문서를 찾는 것이다. Top-5 안에 정답 문서가 존재하면 LLM이 올바른 답변을 생성할 가능성이 높기 때문에 Recall@5를 핵심 지표로 사용하였다.

**Q. 가장 큰 성능 향상 요인은?**

Metadata Filtering과 Document Voting이다. Hard Query 기준 Hit@1을 42%에서 90%까지 향상시켰다.

**Q. 왜 Adaptive Chunking을 선택했나요?**

두 가지 핵심 이유가 있다. 첫째로 문맥 보존 측면에서, 알고리즘 문서의 개념과 관련 설명이 같은 Chunk 안에 함께 유지되기 때문이다. 둘째로 코드 보존 측면에서, 코드 블록이 중간에 잘리지 않고 완전한 형태로 보존되기 때문이다.

**Q. 가장 기억에 남는 시행착오는?**

Hard Query 평가셋 구축 과정이다. 처음에는 "DFS란?", "Heap이란?"처럼 알고리즘 이름이 들어간 질문으로 평가셋을 만들었는데, 실제 사용자 질문과 거리가 멀었다. 알고리즘 이름을 제거하고 "급한 순서대로 처리해야 하는 작업이 있어", "연결된 모든 노드를 한 번씩 방문하려면?" 형태로 바꾸고 나서야 의미 있는 평가가 가능해졌다.

**Q. Metadata 누락을 어떻게 발견했나요?**

Metadata Filtering 결과가 예상보다 적게 나오는 것을 확인하고 ChromaDB에 저장된 메타데이터를 직접 조회하여 발견하였다. algorithm_key가 없거나 대소문자 및 표현 형식이 제각각인 경우가 총 122건이었다. `fix_algorithm_key_metadata.py`를 작성하여 일괄 수정하고 최종 결측치 0건을 달성하였다.

**Q. 향후 개선 계획은?**

BM25 Hybrid Retrieval, BGE-M3, Cross Encoder Reranker를 적용하여 검색 품질을 더욱 향상시킬 계획이다.

...
- KoNLPy 기반 전처리 적용
- 메타데이터 결측치 정제
- Retrieval V2 설계 (LLM Query Rewrite + HyDE)
- Hybrid Router 설계 (V1/V2 자동 분기)
- Full Document Reconstruction 구현
- V1 vs V2 비교 평가
...
│      V1 (규칙 기반)     │      V2 (LLM 기반)     │
│                       │                       │
│  Retrieval Planner    │  LLM Query Rewrite    │
│  Alias Mapping        │  HyDE                 │
│  Metadata Filtering   │  Vector Search        │
│  Query Expansion      │  Document Voting      │
│  MMR Search           │  Full Document        │
│  Document Voting      │    Reconstruction     │
│  Failure Detection    │                       │
│  Fallback Rewrite     │                       │
└───────────────────────┴───────────────────────┘
    ↓
...
메타데이터는 단순 분류 태그가 아닌, **Retrieval 전략에 직접 활용되는 필터 기반**으로 설계하였다. 질문 의도(Intent)에 따라 어떤 메타데이터 필드로 검색 범위를 좁힐지를 먼저 설계한 뒤, 필드를 확정하였다.

> **발표 증빙 준비** : 아래 필드(`contains_code`, `contains_complexity`, `section_order`, `aliases`)는 `splitter.py` 내 Chunk 생성 시점에 실제로 저장되는 값입니다. 평가자가 "실제 코드에서 이 메타데이터가 저장되나요?" 라고 질문하면 `splitter.py`의 해당 코드 블록을 화면에 직접 제시할 수 있도록 준비해두세요.
...
```json
{
  "algorithm": "DFS",
  "algorithm_key": "dfs",
  "aliases": ["깊이우선탐색", "depth first search", "스택 탐색"],

  "category": "graph",
  "difficulty": "beginner",

  "chunk_type": "concept",
  "section_title": "DFS 구현",
  "section_order": 3,

  "contains_code": true,
  "contains_complexity": false,

  "strategy": "recursive",

  "related_topics": ["bfs", "graph", "stack"],
...
**① algorithm_key + aliases**

algorithm_key는 표준화된 소문자 key이며, aliases는 동의어 표현을 배열로 보관한다.

```json
{
  "algorithm_key": "binary_search",
  "aliases": ["이분탐색", "lower bound", "upper bound", "parametric search"]
}
```

사용자가 "이분탐색", "lower bound" 등 다양한 표현을 사용하더라도 동일한 알고리즘 문서를 검색할 수 있도록 한다. Alias Mapping과 연동되어 Retrieval 성능에 직접 기여한다.

---

**② contains_code / contains_complexity**

Chunk 단위로 코드 포함 여부와 시간복잡도 설명 포함 여부를 boolean으로 관리한다.
...
질문 의도에 따라 다음과 같이 필터를 분기할 수 있다.

| 질문 유형 | 적용 필터 |
| ------------------- | --------------------------------- |
| "DFS 구현 코드 보여줘" | `contains_code = True` 우선 검색     |
| "시간복잡도가 뭐야?" | `contains_complexity = True` 필터 적용 |
| "DFS 개념 설명해줘" | `chunk_type = concept` 필터 적용     |

이를 통해 동일한 알고리즘 내에서도 질문 의도에 맞는 Chunk를 정밀하게 검색할 수 있다.

---

**③ section_title + section_order**

섹션 제목과 섹션 순서를 함께 보관한다.
...
section_order를 활용하면 향후 GraphRAG 확장 시 알고리즘 학습 흐름을 순서대로 연결할 수 있다.

```text
section_order 1 : DFS 개념
section_order 2 : DFS 시간복잡도
section_order 3 : DFS 구현
section_order 4 : DFS 예제
```
...
해당 알고리즘을 이해하기 위해 선행 학습이 필요한 개념을 배열로 관리한다.
...
학습 추천 시나리오에서 활용할 수 있다.

```text
사용자 질문: "Binary Search 배우고 싶어요"

→ prerequisites: ["array", "sorting"]
→ "먼저 배열과 정렬을 학습한 후 이분탐색을 진행하세요"
```
...
Adaptive Chunking에서 Chunk 크기 제한을 초과하는 경우에도 코드 블록 내부에서는 절대 분할하지 않는다.
...
기존 Recursive Chunking은 1600자 초과 시 코드 블록 내부에서도 분할이 발생할 수 있었다. Adaptive Chunking은 코드 블록을 단일 단위로 인식하여 항상 완전한 구현 예제를 보존한다.

---

#### 메타데이터 기반 Retrieval 전략 연결 예시
...
↓
Metadata Filter:
  algorithm_key IN ["binary_search", "이분탐색", "lower bound"]
  contains_complexity = True
↓
Binary Search 시간복잡도 Chunk 우선 반환
```

이처럼 메타데이터는 분류 목적이 아닌 **Retrieval 전략의 분기 조건**으로 설계되었다.
...
Adaptive Chunking은 코드 블록(` ``` `)을 하나의 단위로 인식하여 분할 기준으로 삼지 않는다. Chunk 크기 제한을 초과하는 경우에도 코드 블록 내부에서는 절대 분할하지 않는다.
...
Adaptive Chunking은 섹션 단위 분할이기 때문에 각 Chunk에 `chunk_type`, `section_title`, `section_order`, `contains_code`, `contains_complexity` 등의 메타데이터를 정확하게 부여할 수 있다.

```text
DFS 개념 Chunk    → chunk_type=concept,   contains_code=False
DFS 코드 Chunk    → chunk_type=code,      contains_code=True
DFS 복잡도 Chunk  → chunk_type=complexity, contains_complexity=True
```

이를 통해 질문 의도에 따라 필요한 Chunk 유형만 정밀하게 검색하는 Intent 기반 Retrieval이 가능하다.

결과적으로 Adaptive Chunking을 적용한 이후 코드 기반 질의 대응 성능이 향상되었으며, 메타데이터 기반 Retrieval 전략의 설명 가능성도 함께 확보되었다.
...
Retrieval V1은 Hard Query 기준 Hit@1 94%, Recall@5 100%를 달성하였지만, 다음 한계가 존재하였다.

1. 규칙 기반 Alias Mapping은 사전에 등록되지 않은 표현에 대응할 수 없다.
2. 문법 질문, 자연어 중심 질문 등 알고리즘 키워드가 없는 질문에서는 Metadata Filter가 작동하지 않는다.
3. 복합 조건 질문에서 단일 Alias로 의도를 파악하기 어렵다.

Retrieval V2는 이러한 한계를 LLM 기반 Query Rewrite로 해결하기 위해 설계하였다.
...
### 5-A-3. LLM Query Rewrite

사용자의 자연어 질문을 LLM이 검색에 최적화된 형태로 재작성한다.

예시:

```text
원본: 배열에서 연속된 부분합 중 최대값을 구하는 문제인데 DP로 푸는 방법 설명해줘

LLM Rewrite: Dynamic Programming Kadane Algorithm 최대 부분 배열 합
```

V1의 규칙 기반 Alias Mapping과 달리, LLM Rewrite는 사전에 등록되지 않은 표현도 처리할 수 있다.

---

### 5-A-4. HyDE (Hypothetical Document Embedding)

질문을 직접 임베딩하는 대신, LLM이 생성한 가상 답변을 임베딩하여 검색한다.

```text
질문: for 반복문 사용법 알려줘

가상 답변:
for 반복문은 정해진 횟수만큼 코드를 반복 실행하는 구문입니다.
Python에서는 range()와 함께 사용하며...

→ 가상 답변을 임베딩 → 실제 문서와 유사도 비교
```

효과: 질문과 문서 사이의 의미 간극을 줄여 검색 정확도를 높인다.

---

### 5-A-5. Full Document Reconstruction

팀원(LLM 파트) 요구사항: 검색된 chunk만으로는 전체 맥락 파악이 어렵다는 피드백.

해결: voted_docs(대표 chunk)와 별도로, source_file 기준 원본 전체 문서를 복원하여 함께 반환한다.

```text
voted_docs     : 대표 chunk (검색 결과 기반)
full_documents : source_file 기준 원본 전체 문서

정렬 기준: chunk_index / section_order
```

이를 통해 LLM이 chunk 단위 Context와 전체 문서 Context를 모두 활용할 수 있도록 하였다.
...
V1과 V2는 각각 다른 유형의 질문에 강점이 있다.

| 질문 유형 | V1 성능 | V2 성능 | 적합한 버전 |
| -------- | ------ | ------ | -------- |
| 명확한 알고리즘 질문 | 높음 | 보통 | V1 |
| 문법/자연어/애매한 질문 | 낮음 | 높음 | V2 |

따라서 질문 유형에 따라 V1 또는 V2를 자동으로 선택하는 Hybrid Router를 설계하였다.
...
Hybrid Router를 통해 각 질문 유형에 최적화된 Retrieval 전략을 자동 적용할 수 있게 되었다.

```text
"DFS 구현 코드 보여줘"        → V1 (Alias 매칭 즉시 가능)
"for 반복문 사용법 알려줘"     → V2 (LLM Rewrite + HyDE)
"급한 순서대로 처리하려면?"     → V2 (LLM이 priority_queue로 변환)
"이분탐색 시간복잡도"          → V1 (명확한 알고리즘 키워드)
```
...
### app/rag/retrieval_v2.py

LLM 기반 Query Rewrite와 HyDE를 활용한 V2 Retrieval 파이프라인입니다.

V1이 처리하기 어려운 문법 질문, 자연어 중심 질문에 대응합니다.

---

### app/rag/retrieval_router.py

질문 유형에 따라 V1 또는 V2를 자동 선택하는 Hybrid Router입니다.

알고리즘 키워드가 명확하면 V1, 애매하면 V2로 분기합니다.

---

### app/prompts/retrieval_rewrite_prompt.py

LLM Query Rewrite에 사용하는 프롬프트 템플릿을 관리합니다.
...
| Retrieval V1        | 0.920 |    1.000 | 0.960 |
...
| Retrieval V1        | 0.940 |    1.000 | 0.970 |


---

### 9-3. V1 vs V2 Special Evaluation

V1과 V2의 성능을 동일 평가셋으로 비교하였다.

| 지표 | V1 | V2 | 비고 |
| ---- | ---: | ---: | ---- |
| Hit@1 | 0.700 | 0.900 | V2가 +20.0%p |
| Recall@5 | 0.800 | 1.000 | V2 정답 100% 포함 |
| FullDoc Rate | — | 1.000 | 전체 문서 복원 성공 |

해석:

V2는 LLM Query Rewrite를 통해 사전에 등록되지 않은 표현도 처리할 수 있어, V1 대비 Hit@1이 20%p 향상되었다.

Recall@5 = 1.000은 V1과 동일하게 Top-5 안에 정답 문서가 모두 포함됨을 의미한다.

FullDoc Rate = 1.000은 voted_docs에서 선정된 문서의 전체 원본을 100% 복원하여 LLM에 전달했음을 의미한다.

---

### 9-4. Hybrid Router 효과

| 질문 유형 | V1 단독 | V2 단독 | Hybrid Router |
| -------- | ------: | ------: | -----------: |
| 명확한 알고리즘 질문 | 0.940 | 0.860 | 0.940 (V1 선택) |
| 애매한/문법 질문 | 0.420 | 0.900 | 0.900 (V2 선택) |

Hybrid Router는 질문 유형에 따라 최적 버전을 자동 선택하므로, 두 유형 모두에서 최고 성능을 달성한다.
...
| + Document Voting (V1 전체) | 0.940 |    1.000 | 0.970 | **+10.0%p**  |
...
Recall@5 = 1.000이므로 정답 문서 자체를 찾지 못한 케이스는 없다. Hit@1 실패는 **정답 문서가 Top-5 안에 있지만 1위가 아닌 경우**, 즉 Retrieval 실패가 아닌 Ranking 순서 문제이다.

실패 케이스별로 어떤 알고리즘이 정답보다 상위에 위치했는지를 분석하여 Alias 및 Reranker 개선 방향을 도출하였다.
...
```text
Recall@5 = 1.000 → 정답 문서 검색 자체는 100% 성공

Hit@1 실패 유형 분류

1. Alias 부재          : 유사 알고리즘이 정답보다 먼저 매핑됨 → Alias 확장
2. 의미 중복           : 유사 알고리즘 간 임베딩 거리 가까움 → Reranker 도입
3. 복합 조건 질문       : 단일 Alias로 처리 불가, 오답이 상위 랭킹 → LLM Query Rewriting
```

Hit@1 실패 케이스 모두 정답 문서는 Top-5 안에 있었다. 이는 Retrieval 실패가 아니라 Ranking 최적화 문제이며, Cross Encoder Reranker 도입으로 해결 가능하다.
...
```text
실제 저장된 Collection : langchain  (1686 chunks)
검색 시 참조한 이름    : algorithm_docs  ← 이름 불일치
...
실제 저장된 Collection 이름을 직접 조회하여 확인한 뒤 검색 코드의 참조 이름을 통일하였다.
...
| LLM Query Rewrite (V2) | 완료 | LLM 기반 검색 쿼리 최적화                  |
| HyDE (V2)              | 완료 | 가상 답변 임베딩 기반 검색                   |
| Hybrid Router          | 완료 | V1/V2 자동 분기                          |
| Full Document 복원      | 완료 | chunk → 원본 전체 문서 복원                 |
...
3. BM25 Hybrid Retrieval
   ...
   - TF-IDF baseline 대비 성능 비교

4. 한국어/코딩 문서 특화 임베딩 모델 비교
   - BGE-M3
...
구체적으로는 알고리즘 문서 수작업 큐레이션 및 품질 검수, 전처리, 메타데이터 설계 및 표준화, 청킹, 임베딩, ChromaDB 저장, Retrieval V1 설계, Metadata Filtering, Query Expansion, MMR Search, Document Voting, Failure Detection, Fallback Query Rewrite, Retrieval V2 설계 (LLM Query Rewrite + HyDE), Hybrid Router 설계 (V1/V2 자동 분기), Full Document Reconstruction, KoNLPy/TF-IDF baseline, Basic/Hard 평가, V1 vs V2 비교 평가까지 수행했습니다.
...
**Q. Recall@5는 100%인데 왜 Hit@1은 94%인가요?**

정답 문서는 모두 Top-5 안에 포함되었다. Hit@1 실패는 검색 자체의 실패가 아니라, 일부 질의에서 의미적으로 유사한 알고리즘 문서가 정답보다 상위 순위에 위치한 Ranking 최적화 문제이다. 즉 Retrieval은 성공했지만 1위 문서가 정답이 아닌 경우이다. Cross Encoder Reranker를 도입하면 해결 가능하다.

algorithm_key 기반 필터 외에 질문 의도(Intent)에 따라 contains_code, contains_complexity 필드를 추가로 적용한다. 구현 코드 질문이면 `contains_code=True` Chunk를 우선 검색하고, 시간복잡도 질문이면 `contains_complexity=True` Chunk를 우선 반환한다. 메타데이터는 단순 분류 태그가 아니라 Retrieval 전략의 분기 조건으로 설계하였다.

**Q. aliases 필드는 왜 필요한가요?**

사용자는 "이분탐색", "lower bound", "parametric search" 등 다양한 표현을 사용하지만 실제 문서의 algorithm_key는 `binary_search`로 표준화되어 있다. aliases 배열에 동의어를 등록해두면 Alias Mapping이 이를 참조하여 algorithm_key로 변환하므로 표현 형식에 관계없이 동일한 문서를 검색할 수 있다.

**Q. prerequisites 필드는 어디에 활용되나요?**

현재는 메타데이터로 저장만 해두었으며, 향후 학습 추천 API 구현 시 활용할 계획이다. "Binary Search 배우고 싶어요" 질문이 들어오면 prerequisites 필드를 참조하여 "먼저 배열과 정렬을 학습한 후 진행하세요"와 같은 학습 경로를 안내할 수 있다.
...
Ablation Study 결과 Alias Mapping이 단일 기능 중 가장 큰 기여 요소였다. Hit@1 기준 단독으로 +26.0%p 향상되었다. 이후 Metadata Filter +16.0%p, Document Voting +10.0%p가 누적되어 최종 Hard Hit@1 94%를 달성하였다.
...
Recall@5 = 1.000이므로 정답 문서를 아예 찾지 못한 케이스는 없다. Hit@1 실패 케이스를 유형별로 분류하였다. 주요 유형은 Alias 부재로 유사 알고리즘이 정답보다 먼저 매핑된 경우, 유사 알고리즘 간 임베딩 거리가 가까워 오답이 상위에 위치한 경우, 복합 조건 질문에서 단일 Alias로 처리 불가한 경우이다. 모두 Retrieval 실패가 아닌 Ranking 최적화 문제이며, Cross Encoder Reranker 도입으로 해결 가능하다.

**Q. 왜 Adaptive Chunking을 선택했나요?**

세 가지 이유가 있다. 첫째 문맥 보존, 둘째 코드 블록 분할 금지(분할 우선순위 적용), 셋째 섹션 단위 분할 덕분에 `chunk_type`, `contains_code` 등의 메타데이터를 정확하게 부여할 수 있어 Intent 기반 Retrieval이 가능하다는 점이다.
...
Hybrid Router의 분기 기준을 정교화하고, Cross Encoder Reranker를 도입하여 Hit@1 실패 케이스를 해결할 계획이다. 이후 BM25 Hybrid Retrieval, BGE-M3 임베딩 교체, Graph RAG 순으로 고도화할 예정이다.

**Q. V1과 V2의 차이는?**

V1은 규칙 기반 Alias Mapping으로 빠르고 정확하지만, 사전에 등록되지 않은 표현에 대응할 수 없다. V2는 LLM Query Rewrite + HyDE로 유연하지만 LLM 호출 비용이 발생한다. Hybrid Router가 질문 유형에 따라 최적 버전을 자동 선택한다.

**Q. Full Document Reconstruction은 왜 필요한가?**

팀원(LLM 파트)으로부터 검색된 chunk만으로는 전체 맥락 파악이 어렵다는 피드백을 받았다. voted_docs는 대표 chunk를 반환하고, full_documents는 source_file 기준 원본 전체 문서를 chunk_index/section_order 기준으로 정렬하여 함께 반환한다. LLM이 두 가지 수준의 Context를 모두 활용할 수 있다.

**Q. HyDE가 일반 검색보다 나은 이유는?**

일반 검색은 질문("for문 사용법 알려줘")과 문서("for 반복문은 정해진 횟수만큼...")의 의미 간극이 크다. HyDE는 LLM이 생성한 가상 답변을 임베딩하므로, 문서와 동일한 형태의 텍스트로 검색하게 되어 유사도가 높아진다.

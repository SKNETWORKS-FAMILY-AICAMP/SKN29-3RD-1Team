...
사용 모델: `get_qwen_model("think")` (현재 OpenAI GPT-4o-mini로 대체)
...
사용 모델: `get_qwen_model("query")` (현재 OpenAI로 대체)
...
| 노드 | 함수 | 실제 모델 | 비고 |
|------|------|-----------|------|
| think | `get_qwen_model("think")` | GPT-4o-mini | DISABLE=True로 OpenAI 대체 |
| generate_code | `get_openai_model()` | GPT-4o-mini | temperature=0 |
| debug | `get_openai_model()` | GPT-4o-mini | temperature=0 |
| algorithm_query | `get_qwen_model("query")` | GPT-4o-mini | DISABLE=True로 OpenAI 대체 |
| final_answer | `get_openai_model()` | GPT-4o-mini | temperature=0 |

`qwen_model.py`의 `DISABLE = True` 설정 시 모든 Qwen 호출이 OpenAI로 대체된다.

로컬 GPU 환경이 준비되면 `DISABLE = False`로 전환하여 LoRA 어댑터가 적용된 Qwen3-0.6B를 사용할 수 있다.
...
- `exec(code, {})`: 빈 네임스페이스로 실행하여 전역 상태 오염 방지
- `redirect_stdout`: print 출력을 캡처하여 반환
- `io.StringIO(stdin)`: 표준 입력을 문자열로 주입 (input() 대응)
- 오류 발생 시 `ERROR: <메시지>` 반환

한계: 무한루프, 메모리 초과, 파일 시스템 접근에 대한 제한이 없다.

### 현재 호출 방식

현재는 MCP 프로토콜을 거치지 않고 `execute_python`을 직접 임포트하여 호출한다.

```python
# solve_agent.py 현재 방식
from app.mcp.tools import execute_python
result = execute_python(code=state["code"], stdin=state["test_input"])
```

`tools.py`에는 `@mcp.tool()` 데코레이터가 선언되어 있으나, FastMCP 서버는 아직 별도 프로세스로 기동되지 않는다.

### MCP 서버 연동 (추가 예정)

> **TODO**: MCP 서버가 도입되면 아래 내용을 채워넣을 것.
>
> - MCP 서버 기동 방식 및 포트 설정
> - 에이전트가 MCP 클라이언트로 tool을 호출하는 구조 변경 내용
> - 추가될 MCP tool 목록
...
| Qwen 비활성화 | DISABLE=True 상태로 모든 노드가 OpenAI 사용 |
...
4. **Qwen 어댑터 활성화**: 로컬 GPU 환경 구성 후 think/query 노드에 Qwen LoRA 적용

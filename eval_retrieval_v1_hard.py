...
def load_eval_cases(dataset_path: str = DEFAULT_DATASET_PATH) -> List[Dict[str, Any]]:
    ...
    if not path.exists():
        raise FileNotFoundError(
            ...
            f"현재 위치에서 실행 중인지 확인하세요. 예: python eval_retrieval_v1_hard.py"
        )
...
HARD_EVAL_CASES = load_eval_cases(DATASET_PATH)
...
def evaluate_plain_vector():
    ...
    n = len(HARD_EVAL_CASES)
    return {
        "name": "Plain Vector Search - Hard",
        "hit1": hit1 / n,
        "recall5": hit5 / n,
        "mrr": mrr_sum / n,
        "rows": rows,
...
def evaluate_retrieval_v1():
    ...
    n = len(HARD_EVAL_CASES)
    return {
        "name": "Retrieval V1 Metadata Filter + Voting - Hard",
        "hit1": hit1 / n,
        "recall5": hit5 / n,
        "mrr": mrr_sum / n,
        "rows": rows,
...
if __name__ == "__main__":
    print("\n" + "=" * 100)
    print("Retrieval V1 Hard Query Evaluation")
    print("=" * 100)
    print(f"Dataset : {DATASET_PATH}")
    print(f"Cases   : {len(HARD_EVAL_CASES)}")
